...
async function executeWithRetry(tx: Transaction, description: string) {
  for (let attempt = 1; attempt <= 5; attempt++) {
    try {
      return await suiClient.signAndExecuteTransaction({ transaction: tx, signer: keypair });
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : String(e);
      if (msg.includes('Too Many Requests') && attempt < 5) {
        console.log(`Rate limited on ${description}, retrying in ${attempt * 3}s...`);
  ...
  throw new Error(`${description} failed after 5 attempts`);
}
...
async function claimFlag(address: string): Promise<boolean> {
  ...
  const result = await executeWithRetry(tx, 'claim_flag');
...
(async () => {
  ...
  if (!player) {
    ...
    try {
      ...
      const result = await executeWithRetry(tx, 'register');
  ...
  while (true) {
    ...
    // Only scan opponents if we're potentially competitive enough to attack
    // (avoids 200 parallel RPC calls that trigger rate limits when we'd never attack anyway)
    let opponents: PlayerEntry[] = [];
    if (player.shield >= 6) {
      opponents = (await scanPlayers(address)).sort((a, b) => b.state.shield - a.state.shield);
    }
    ...
    if (shouldAttack) {
      ...
      for (const opponent of opponents) {
        ...
        try {
          const result = await executeWithRetry(tx, 'attack');
          if (result.Transaction) {
      ...
      if (!attacked) {
        ...
        const result = await executeWithRetry(tx, 'build');
